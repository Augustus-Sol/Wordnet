...
def findWord(s):
    ...
    for loo in range(8):
        ...
        while (defResult != [] or exSentence != []):
            ...
            if partOfSpeech:
                strList.append(list(partOfSpeech).pop())
            if defResult:
                strList.append(list(defResult).pop())
            if exSentence:
                strList.append(list(exSentence).pop())
            if synList:
            ...
            if strList:
                print(strList)
                print('\n')
                masterList.append(strList)  # appends the database row to our masterList 'table'

    if masterList:
        file_to_write = open(str(s)+'_ALL.txt', 'w')
